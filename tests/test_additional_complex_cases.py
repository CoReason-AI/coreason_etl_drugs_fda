...
def test_missing_submissions_file() -> None:
    ...
    with patch("dlt.sources.helpers.requests.get") as mock_get:
        ...
        assert "fda_drugs_bronze_fda_products" in resource_names
        ...
        # So Gold IS yielded.
        assert "fda_drugs_gold_drug_product" in resource_names

        # Verify Gold content - should have null approval dates
        gold_res = source.resources["fda_drugs_gold_drug_product"]
        rows = list(gold_res)
        assert len(rows) == 1
