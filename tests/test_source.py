...
def test_drugs_fda_source_extraction(mock_zip_content: bytes) -> None:
    ...
    with patch("dlt.sources.helpers.requests.get") as mock_get:
        ...
        raw_prod = list(resources["fda_drugs_bronze_fda_products"])
...
def test_source_skips_silver_if_missing_files() -> None:
    ...
    with patch("dlt.sources.helpers.requests.get") as mock_get:
        ...
        assert "fda_drugs_gold_drug_product" in resources  # Should be present (only depends on Products)
    ...
    with patch("dlt.sources.helpers.requests.get") as mock_get:
        ...
        assert "fda_drugs_gold_drug_product" not in resources
